...
} from "../../../redux/features/storyPackages/storyPackages";
...
const emptyForm = { name: "", description: "", durationHours: "", price: "", maxStories: 1, currency: "USD", validityDays: 30 };
...
const PackageCard = ({ pkg, onEdit, onDelete, onToggle }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 w-72 relative shadow-sm border border-gray-100 flex flex-col justify-between">
      ...
      <p className="text-[12px] text-gray-400 mb-3 line-clamp-2">{pkg.description}</p>

      <div className="space-y-1 text-sm text-gray-600 mb-3">
        <p>Duration: <span className="font-medium text-gray-800">{pkg.durationHours}h</span></p>
        <p>Max Stories: <span className="font-medium text-gray-800">{pkg.maxStories}</span></p>
...
/* ---------------- Main ---------------- */
const StoryPackages = () => {
  const { data: packages, isLoading, isError } = useGetStoryPackagesQuery();
  const [addStoryPackage] = useAddStoryPackageMutation();
  const [updateStoryPackage] = useUpdateStoryPackageMutation();
  const [deleteStoryPackage] = useDeleteStoryPackageMutation();
  const [toggleStatus] = useToggleStoryPackageStatusMutation();
  ...
  const handleAdd = async () => {
    try {
      ...
      toast.success("Story package created");
      setAddOpen(false);
      setForm(emptyForm);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await deleteStoryPackage(id).unwrap();
  ...
  const renderModal = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.45)" }} onClick={closeModal}>
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-lg font-bold text-gray-800 mb-5">{editItem ? "Edit" : "Add"} Story Package</h2>
        ...
        <textarea className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-4 h-24 resize-none focus:outline-none focus:ring-2 focus:ring-green-700 bg-gray-50" placeholder="Enter description" value={form.description} onChange={(e) => setField("description", e.target.value)} />
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div>
            ...
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-700 bg-gray-50" type="number" placeholder="7.99" value={form.price} onChange={(e) => setField("price", e.target.value)} />
          </div>
        </div>

        <div className="grid grid-cols-3 gap-3 mb-6">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Max Stories</label>
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-700 bg-gray-50" type="number" placeholder="1" value={form.maxStories} onChange={(e) => setField("maxStories", e.target.value)} />
          </div>
          <div>
  ...
  return (
    <section className="py-7">
      <div className="flex justify-between items-start gap-4 mb-6 pl-1">
        <div>
          <h2 className="text-[22px] font-bold text-gray-900 mb-1">Story Packages</h2>
          <p className="hidden lg:block text-[16px] text-gray-400">Manage story pricing packages.</p>
        </div>
