...
import { useGetUserSubscriptionStatsQuery } from "../../../redux/features/userSubscriptions/userSubscriptions";

const UserSubscriptionsStats = () => {
  const { data } = useGetUserSubscriptionStatsQuery();

  const stats = [
    { label: "Total Subscriptions", value: data?.totalSubscriptions ?? 0, icon: <FiShoppingBag size={20} />, bg: "bg-blue-50", color: "text-blue-600" },
    { label: "Active", value: data?.activeSubscriptions ?? 0, icon: <FiCheckCircle size={20} />, bg: "bg-green-50", color: "text-green-600" },
    { label: "Expired", value: data?.expiredSubscriptions ?? 0, icon: <FiAlertCircle size={20} />, bg: "bg-yellow-50", color: "text-yellow-600" },
    { label: "Cancelled", value: data?.cancelledSubscriptions ?? 0, icon: <FiXCircle size={20} />, bg: "bg-red-50", color: "text-red-500" },
...
export default UserSubscriptionsStats;
